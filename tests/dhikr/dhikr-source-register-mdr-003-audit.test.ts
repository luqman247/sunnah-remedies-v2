...
function testOnlyMdr003ResearchFieldsChanged() {
  // MDR-004 through MDR-007 are excluded from this comparison: each was
  ...
  // -mdr-005-audit.test.ts, -mdr-006-audit.test.ts, and -mdr-007-audit.test.ts.
  const excludedIds = new Set(["MDR-004", "MDR-005", "MDR-006", "MDR-007"]);
  ...
  assert(
    ...
    `Expected ${baseline.length} records besides MDR-003/MDR-004/MDR-005/MDR-006/MDR-007, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-003 changed in this stage; MDR-001, MDR-002, and MDR-008 through MDR-030 match checkpoint 5880e17 exactly (MDR-004 through MDR-007 verified separately)",
  );
}
...
function testMdr008Through030RemainUnchanged() {
  // MDR-004 through MDR-007 are excluded: each was legitimately researched
  ...
  // checks MDR-008 through MDR-030 against the Stage 3A checkpoint used
  ...
  console.log("✓ MDR-008 through MDR-030 remain unchanged (23 records checked; MDR-004 through MDR-007 verified separately)");
}

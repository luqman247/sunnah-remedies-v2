...
function testOnlyMdr003ResearchFieldsChanged() {
  // MDR-004, MDR-005, and MDR-006 are excluded from this comparison: each
  ...
  // by their own dedicated files, tests/dhikr/dhikr-source-register-mdr-004-audit.test.ts,
  // -mdr-005-audit.test.ts, and -mdr-006-audit.test.ts.
  const excludedIds = new Set(["MDR-004", "MDR-005", "MDR-006"]);
  ...
  assert(
    ...
    `Expected ${baseline.length} records besides MDR-003/MDR-004/MDR-005/MDR-006, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-003 changed in this stage; MDR-001, MDR-002, and MDR-007 through MDR-030 match checkpoint 5880e17 exactly (MDR-004, MDR-005, and MDR-006 verified separately)",
  );
}
...
function testMdr007Through030RemainUnchanged() {
  // MDR-004, MDR-005, and MDR-006 are excluded: each was legitimately
  ...
  // This function checks MDR-007 through MDR-030 against the Stage 3A
  ...
  console.log("✓ MDR-007 through MDR-030 remain unchanged (24 records checked; MDR-004, MDR-005, and MDR-006 verified separately)");
}

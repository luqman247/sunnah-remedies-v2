...
function testOnlyMdr006ResearchFieldsChanged() {
  ...
  assert(
    ...
    `Expected ${baseline.length} records besides MDR-006/MDR-007/MDR-008/MDR-009/MDR-010–030, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-006 changed in this stage; MDR-001 through MDR-005 match checkpoint 38a0600 exactly (MDR-007/MDR-008/MDR-009/MDR-010–020/MDR-021–030 verified separately)",
  );
}
...
// MDR-010 through MDR-030 are entirely covered by testOnlyMdr006ResearchFieldsChanged's
...
function testComputeImportGateRemainsFalse() {
  ...
  assert(
    ...
    `MDR-006 should remain blocked by exactly thirteen independent conditions (Stage 2 strengthened the gate with operational approval checks), found ${gate.blockedReasons.length}: ${gate.blockedReasons.join(" | ")}`,
  );
  ...
  console.log(`✓ computeImportGate(MDR-006) remains false with thirteen blockers (source research, wording, scholarly approval, research-only status, plus Stage 2's operational approval-field checks) — the canonical gate was not weakened`);
}

...
function testOnlyMdr005ResearchFieldsChanged() {
  // MDR-006 and MDR-007 are excluded from this comparison: each was
  ...
  // and -mdr-007-audit.test.ts.
  const excludedIds = new Set(["MDR-006", "MDR-007"]);
  ...
  assert(
    ...
    `Expected ${baseline.length} records besides MDR-005/MDR-006/MDR-007, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-005 changed in this stage; MDR-001 through MDR-004 and MDR-008 through MDR-030 match checkpoint 3729062 exactly (MDR-006 and MDR-007 verified separately)",
  );
}
...
function testMdr008Through030RemainUnchanged() {
  // MDR-006 and MDR-007 are excluded: each was legitimately researched in
  ...
  console.log("✓ MDR-008 through MDR-030 remain unchanged (23 records checked; MDR-006 and MDR-007 verified separately)");
}

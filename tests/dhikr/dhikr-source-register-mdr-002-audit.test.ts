...
function testOnlyMdr002ChangedInThisStage() {
  // MDR-003 through MDR-008 are excluded from this comparison: each was
  ...
  const excludedIds = new Set(["MDR-003", "MDR-004", "MDR-005", "MDR-006", "MDR-007", "MDR-008"]);
  ...
  assert(
    ...
    `Expected ${baseline.length} records besides MDR-002/MDR-003/MDR-004/MDR-005/MDR-006/MDR-007/MDR-008, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-002 changed in this stage; MDR-001 and MDR-009 through MDR-030 match checkpoint 8e2c46d exactly (MDR-003 through MDR-008 verified separately)",
  );
}

...
function testOnlyMdr004ResearchFieldsChanged() {
  // MDR-005 through MDR-008 are excluded from this comparison: each was
  ...
  // -mdr-006-audit.test.ts, -mdr-007-audit.test.ts, and -mdr-008-audit.test.ts.
  const excludedIds = new Set(["MDR-005", "MDR-006", "MDR-007", "MDR-008"]);
  ...
  assert(
    ...
    `Expected ${baseline.length} records besides MDR-004/MDR-005/MDR-006/MDR-007/MDR-008, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-004 changed in this stage; MDR-001 through MDR-003 and MDR-009 through MDR-030 match checkpoint b428838 exactly (MDR-005 through MDR-008 verified separately)",
  );
}
...
function testMdr009Through030RemainUnchanged() {
  // MDR-005 through MDR-008 are excluded: each was legitimately researched
  ...
  console.log("✓ MDR-009 through MDR-030 remain unchanged (22 records checked; MDR-005 through MDR-008 verified separately)");
}

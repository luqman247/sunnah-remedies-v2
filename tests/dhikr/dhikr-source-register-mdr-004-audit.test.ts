...
function testOnlyMdr004ResearchFieldsChanged() {
  ...
  const otherRecords = REGISTER.filter((r) => r.internalId !== "MDR-004");
  assert(
    otherRecords.length === baseline.length,
    `Expected ${baseline.length} records besides MDR-004, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-004 changed in this stage; MDR-001 through MDR-003 and MDR-005 through MDR-030 match checkpoint b428838 exactly",
  );
}
...
function testMdr005Through030RemainUnchanged() {
  const baseline = loadBaselineFixture();
  ...
  console.log("✓ MDR-005 through MDR-030 remain unchanged (26 records checked)");
}
...
function runAll() {
  ...
  testMdr005Through030RemainUnchanged();

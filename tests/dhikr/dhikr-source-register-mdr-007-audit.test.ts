 ...
 * at the time of MDR-007's own checkpoint) unchanged. MDR-007 was not
 * segmented (see docs/dhikr/research/MDR-007-source-audit.md, "Segmentation
 * decision") — the text has no internal punctuation and no boundary
 * indicator, so no clause-map file exists for this record. All other
 * records are checked against a fixture snapshot captured from checkpoint
 * 8cc2e73 — see
 * tests/dhikr/fixtures/mdr-001-006-008-030-8cc2e73-baseline.json.
 *
 * MDR-008, MDR-009, and MDR-010 through MDR-020 are all excluded from every
 * "unchanged since 8cc2e73" comparison below: each was legitimately
 * researched in a later Stage 3B pass and is expected to differ from this
 * Stage-3A/early baseline. Each is verified separately by its own dedicated
 * test file.
 *
...
function testOnlyMdr007ResearchFieldsChanged() {
  // MDR-008, MDR-009, and MDR-010–020 are excluded from this comparison:
  // each was legitimately researched in a later stage and is no longer
  // expected to match this checkpoint's baseline. Those later changes are
  ...
  const otherRecords = REGISTER.filter((r) => r.internalId !== "MDR-007" && !laterResearchedIds.has(r.internalId));
  assert(
    otherRecords.length === baseline.length,
    `Expected ${baseline.length} records besides MDR-007/MDR-008/MDR-009/MDR-010–030, found ${otherRecords.length}`,
  );
  ...
  console.log(
    "✓ only MDR-007 changed in this stage; MDR-001 through MDR-006 match checkpoint 8cc2e73 exactly (MDR-008/MDR-009/MDR-010–020/MDR-021–030 verified separately)",
  );
}
...
// MDR-010 through MDR-030 are entirely covered by testOnlyMdr007ResearchFieldsChanged's
...
function testComputeImportGateRemainsFalse() {
  ...
  assert(
    ...
    `MDR-007 should remain blocked by exactly thirteen independent conditions (Stage 2 strengthened the gate with operational approval checks), found ${gate.blockedReasons.length}: ${gate.blockedReasons.join(" | ")}`,
  );
  ...
  console.log(`✓ computeImportGate(MDR-007) remains false with thirteen blockers (source research, wording, scholarly approval, research-only status, plus Stage 2's operational approval-field checks)`);
}

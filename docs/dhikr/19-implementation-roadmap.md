...
## Phase 6 (repository) — First real-content phase: Morning Dhikr source register

**Overall status: in progress.** Stage 3A is complete. This phase is distinct from Phase 5 above — it concerns transcription and research staging of real content, not Studio editorial tooling, and has made no Sanity schema, public-route, or public-projection change.

### Stage 3A — Typed research register and verbatim transcription
**Status: complete.** `src/lib/dhikr-research/` added (`types.ts`, `morning-dhikr-register.ts`, `validation.ts`) holding a 30-record, order-protected transcription of the content owner's supplied source document, entirely outside Sanity — see ADR-020 in [21-decision-log.md](21-decision-log.md) and [32-morning-dhikr-source-register.md](32-morning-dhikr-source-register.md) for full methodology, the transcription audit table, and the proposed Stage 3B source-audit process. Every record is deliberately blocked from import by `computeImportGate()`; no source reference, hadith grading, wording verification, repetition/virtue evidence, or scholarly decision was added. No Sanity document type was created and no existing schema, public route, projection, or the canonical eligibility gate was touched — confirmed both by direct inspection and by `tests/dhikr/dhikr-source-register.test.ts`'s 14 static/behavioural checks.

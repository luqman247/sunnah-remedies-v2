...
**Overall status: in progress.** Stages 1, 2, and 3 are complete; Stage 4 (category route scaffolding) is not yet complete. The Knowledge Library integration as a whole must not be treated as complete until every approved stage below, and its verification steps, have finished.
...
**Status: not started.** Planned: `/knowledge-library/dhikr/[category]`, `generateStaticParams` from `getDhikrCategoriesPublic()` (empty today), `notFound()` for any category without at least one eligible published item. Item-detail route (`[category]/[slug]`) remains deferred per ADR-015 and is not part of this stage.

...
## ADR-017 — Category route investigated and deferred, not shipped

**Status**: Decided — deferred, not abandoned
**Context**: a `/knowledge-library/dhikr/[category]` route was implemented (Stage 4), resolving categories exclusively via `getDhikrCategoriesPublic()`, with `notFound()` for any unknown or ineligible category and zero hard-coded category data. It was investigated end-to-end in production mode (`next start`) rather than assumed correct from a passing build.
**Finding**: the route never crashes and never leaks governance, reviewer, or placeholder content, but an unknown or ineligible category slug returns HTTP 200 with correctly-rendered not-found content instead of a genuine HTTP 404. This was traced to a **pre-existing, sitewide** Next.js/next-intl interaction: any in-component `notFound()` call inside a route nested under `src/app/[locale]/` loses its 404 status once next-intl's middleware has rewritten the request — confirmed identically on the existing, unrelated `/knowledge-library/[slug]` and `/the-apothecary/[slug]` routes (both pre-dating this work), and confirmed absent on routes outside `src/app/[locale]/` (e.g. `/knowledge/[type]/[slug]`, which returns a correct 404). No route anywhere in this repository currently demonstrates a working pattern for a true 404 from inside `src/app/[locale]/`. The condition was proven independent of `generateStaticParams` and `export const dynamic = "force-dynamic"` by removing both in an isolated experiment — the 200 status persisted either way. A separate, genuine defect (`DYNAMIC_SERVER_USAGE`, a 500) caused by `generateStaticParams` returning an empty array was found and fixed during the same investigation, but is independent of the 404-status finding.
**Decision**: the category route is not shipped. Given zero eligible public Dhikr categories exist today, shipping it would provide no reader benefit while leaving an ambiguous (soft-404) URL live — avoidable SEO risk for no present gain. Fixing the underlying middleware/next-intl routing behaviour is out of scope for this feature: it is shared infrastructure affecting routes this initiative does not own, and altering `middleware.ts` or next-intl's routing config to chase a single route's correctness was explicitly ruled out.
...
## ADR-020 — Research register lives outside Sanity, as a plain typed array, until scholarly research is complete

**Status**: Decided (Stage 3A, `feature/morning-dhikr-source-register`)
**Decision**: the 30 transcribed Morning Dhikr entries are held in `src/lib/dhikr-research/morning-dhikr-register.ts` — a version-controlled TypeScript array — not as a new `dhikrSourceResearch` Sanity document type, and not as content on `dhikrItem`. `computeImportGate()` in `src/lib/dhikr-research/validation.ts` is the single point that will later decide whether a record may move into `dhikrItem`; no such transition happened in this stage.
**Rationale**: `dhikrItem` (see ADR-006, ADR-010) is built around the assumption that any populated content field is a step closer to publication, and its readiness panel/badges (ADR-018) surface that directly to staff. Loading 30 unresearched transcriptions into Sanity — even in a `sourced`/draft state — would put unverified religious text inside the same system that stages content for publication, one accidental status change away from the compound eligibility gate. A plain, version-controlled array carries no such adjacency: it cannot be opened in Studio, cannot be flipped to `published`, and is provably disconnected from every public/staff route and the canonical gate by static test (see [32-morning-dhikr-source-register.md](32-morning-dhikr-source-register.md), `testNoPublicRouteProjectionOrCanonicalGateChanges`/`testNoSanityDocumentIsCreated`).
**Order-protection design**: rather than relying on a single sort-based helper (which only proves a *derived view* is correct), two independent assertions were implemented — `assertCompleteSequence` (the set of sequence numbers is exactly 1–30) and `assertRegisterStoredInAuthoritativeOrder` (the array's own physical storage order is 1–30) — because a register could pass the former while still being stored out of order, which the source document's own editorial sequencing (stated as authoritative by the content owner) does not permit.
**Mapping to `dhikrItem`**: deferred to Stage 3B. `computeImportGate()` already encodes every blocking condition (unverified source research, unresolved wording, missing required grading, unauthenticated repetition/virtue claims, un-reviewed action-reminder classification, pending scholarly decision, `research-only` status) — the field-by-field mapping from a cleared record into `dhikrItem`'s schema is not yet designed, since no record has ever cleared the gate.
**Compatibility check performed**: `tests/dhikr/dhikr-source-register.test.ts` statically confirms `src/sanity/schemas/documents/dhikr/` contains only the two pre-existing schema files, and that none of the canonical gate, public fetch, public query, or `/dhikr-review` files reference the new module.
